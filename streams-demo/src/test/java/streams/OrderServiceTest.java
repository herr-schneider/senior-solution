...
class OrderServiceTest {
    ...
    @Test
    void counterTest(){
        long num = ordersService.countOrdersByStatus("pending");
    ...
    void collectOrdersWithProductCategory(){
        List<Order> resultIT = ordersService.collectOrdersWithProductCategory("IT");
        List<Order> resultBook = ordersService.collectOrdersWithProductCategory("Book");

        assertEquals(1, resultBook.size());
        assertEquals(2, resultIT.size());

        assertEquals("IT", resultIT.get(0).getProducts().get(0).getCategory());
